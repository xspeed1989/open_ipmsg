...
cargo run -- --selftest    # 无头互通自检（12 项 PASS）
```

`--selftest` 会在本机回环地址启动完整网络栈，并内置一个"假对端"完成：
发现注册 → 双向文本（含已读回执）→ 发送附件（对端经 TCP 取回逐字节比对）→
接收附件（从对端 TCP 服务下载比对）→ BR_EXIT 下线广播，全部通过后退出码为 0。

## 使用说明

1. 启动后首次运行弹出设置：填写**昵称**（必填）与**群组**（选填），保存即向局域网广播上线
2. 局域网内其他 IPMsg 客户端（官方 Windows 版、Mac 版、iptux 等）会出现在「通讯录」
3. 点击联系人发起会话；输入框 **Enter 发送 / Ctrl+Enter 换行**
4. 工具栏 📎 选择附件发送；对方发来的附件点「下载」，完成后可「打开」或定位文件夹
5. 接收目录默认为应用数据目录下的 `接收文件`，可在设置中修改
